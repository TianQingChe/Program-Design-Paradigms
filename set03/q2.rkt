...
;; IMPLEMENTATION:
(define-struct world (ball racket speed state tick-number))
...
(define RALLY-BALL (make-ball 3 -9 READY-X READY-Y))
...
(define BALL-BOTTOM-WORLD-AT-1
  (make-world (make-ball 3 9 220 645) READY-RACKET ONE RALLY
               NEG-ONE))
...
(define (make-rally-world w)
  (if (= (world-tick-number w) -2)
      (make-world
           (ball-after-tick
           (world-racket w) (world-ball w) (world-state w)
           (world-tick-number w))
...
;;; ball-after-tick:Racket Ball World-state World-tick-number-> Ball
;;; GIVEN: the state of a ball b, a racket r, world-state w-s of a
;;;        world
...
(define (ball-after-tick r b w-s wtn)
  (cond
...
(define RACKET-FOR-TEST (make-racket 0 5 100 100 FALSE -10 -10 -10
                                     -10))
...
(define BALL-AFTER-COLLIDE-WITH-RACKET (make-ball 3 14 93 124))
...
(begin-for-test
  (check-equal? (ball-collides-with-racket? RACKET-FOR-TEST
                 BALL-COLLIDE-RACKET RALLY NEG-ONE) TRUE)
  (check-equal? (ball-collides-with-racket? RACKET-FOR-TEST
                 BALL-COLLIDE-RIGHT RALLY NEG-ONE) FALSE))
...
(begin-for-test
  (check-equal? (ball-collides-front-wall? RACKET-FOR-TEST
                                 BALL-COLLIDE-FRONT RALLY NEG-ONE) TRUE)
  (check-equal? (ball-collides-front-wall? RACKET-FOR-TEST
                                 BALL-COLLIDE-RIGHT RALLY NEG-ONE) FALSE))
...
(begin-for-test
  (check-equal? (ball-collides-bottom-wall? RACKET-FOR-TEST
                                 BALL-COLLIDE-BOTTOM RALLY NEG-ONE) TRUE)
  (check-equal? (ball-collides-bottom-wall? RACKET-FOR-TEST
                                 BALL-COLLIDE-RIGHT RALLY NEG-ONE) FALSE))
...
(begin-for-test
  (check-equal? (ball-collides-right-wall? RACKET-FOR-TEST
                                 BALL-COLLIDE-RIGHT RALLY NEG-ONE) TRUE)
  (check-equal? (ball-collides-right-wall? RACKET-FOR-TEST
                                 BALL-COLLIDE-LEFT RALLY NEG-ONE) FALSE))
...
;;;                  ball and walls
(define (racket-after-tick r b w-s wtn)
   (cond
     [(and
       (ball-collides-with-racket? r b w-s wtn) (< (racket-vy r) ZERO))
