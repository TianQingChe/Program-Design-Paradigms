...
(define RACKET-FOR-TEST
  (make-racket 0 5 100 100 FALSE -10 -10 -10 -10))

(define RACKET-NO-COLLIDE
  (make-racket 0 5 200 200 FALSE -10 -10 -10 -10))

(define RACKET-COLLIDE-BALL
  (make-racket 0 -5 100 100 FALSE -10 -10 -10 -10))

(define RACKET-COLLIDE-LEFT
  (make-racket -5 0 2 100 FALSE -10 -10 -10 -10))

(define RACKET-COLLIDE-RIGHT  
  (make-racket 5 0 423 100 FALSE -10 -10 -10 -10))

(define RACKET-COLLIDE-FRONT
  (make-racket 0 -5 100 2 FALSE -10 -10 -10 -10))

(define RACKET-COLLIDE-BOTTOM
  (make-racket 0 5 100 646 FALSE -10 -10 -10 -10))

(define RACKET-COLLIDE-LEFT-BOTTOM
  (make-racket -5 5 2 646 FALSE -10 -10 -10 -10))

(define RACKET-COLLIDE-RIGHT-BOTTOM
  (make-racket 5 5 423 646 FALSE -10 -10 -10 -10))

(define RACKET-AFTER-NO-COLLIDE
  (make-racket 0 5 200 205 FALSE -10 -10 -10 -10))

(define RACKET-AFTER-COLLIDE-WITH-BALL
  (make-racket 0 0 100 95 FALSE -10 -10 -10 -10))

(define RACKET-AFTER-COLLIDE-WITH-LEFT
  (make-racket -5 0 24 100 FALSE -10 -10 -10 -10))

(define RACKET-AFTER-COLLIDE-WITH-RIGHT
  (make-racket 5 0 401 100 FALSE -10 -10 -10 -10))

(define RACKET-AFTER-COLLIDE-WITH-BOTTOM
  (make-racket 0 0 100 649 FALSE -10 -10 -10 -10))

(define RACKET-AFTER-COLLIDE-LEFT-BOTTOM
  (make-racket -5 5 24 649 FALSE -10 -10 -10 -10))

(define RACKET-AFTER-COLLIDE-RIGHT-BOTTOM
  (make-racket 5 5 401 649 FALSE -10 -10 -10 -10))
...
;;; GIVEN: the state of a balllist bl, a racket r, world-state w-state of a
;;;        world, world-tick-number wtn
...
;(define (balls-after-tick r bl w-state wtn)
;  (cond
;    [(empty? bl) empty]
;    [else (if (ball-collides-bottom-wall? r (first bl) w-state wtn)
;              (balls-after-tick r (rest bl) w-state wtn)
;              (cons (ball-after-collide r (first bl) w-state wtn)
;                (balls-after-tick r (rest bl) w-state wtn)))]))
;;; DESIGN STRATEGY: Use HOF filter on balllist, followed by HOF map
(define (balls-after-tick r bl w-state wtn)
  (map
   ;; Ball -> Ball
   ;; RETURNS: the ball state after collide with walls or racket
   (lambda (b) (ball-after-collide r b w-state wtn))
       (filter
        ;; Ball -> Boolean
        ;; RETURNS: true if the ball does not collide with the bottom wall
        (lambda (x) (not (ball-collides-bottom-wall? r x w-state wtn)))
        bl)))
...
;;; DESIGN STRATEGY: Use HOF ormap on balllist
(define (balls-collides-with-racket? r bl w-s wtn)
  (ormap
   ;; Ball -> Boolean
   ;; RETURNS: true if the ball collides with the racket
   (lambda (b) (ball-collides-with-racket? r b w-s wtn))
    bl))
...
(begin-for-test
  ...
  (check-equal?
   ...
   (make-world
    (list (make-ball 3 -9 100 300))
    (make-racket ZERO -8 150 400 FALSE 135 380 -10 -10) ONE RALLY
    NEG-ONE))
...
;;; RETURNS: true iff the given coordinate is s positioned no more 
;;;          than 25 pixels away from the center of the racket 
...
(begin-for-test
  (check-equal?
   ...
   "The mouse is not around the racket, the result should be false")
  (check-equal?
   (around-racket? RACKET-FOR-AROUND-TEST 135 380) TRUE
   "The mouse is around the racket, the result should be true"))
